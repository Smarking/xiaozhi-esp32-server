...
1. 同步 override 到 ECS：`docker-compose.override.yml` 含 jaeger 服务 + server 的 env + `image: server_local` 安全阀（见踩坑）。`sync-python.sh` **不含** override，需单独 rsync。
2. 代码同步 → `deploy/sync-python.sh`（`core/` 已在白名单，`telemetry.py` 自动同步）。
...
## 踩坑结论
- **override 必须保留 `image: server_local` 行**：`rebuild-server.sh` 用通配 sed 改 `image:` 行（第 50-51 行）。若 server 段没有 image 行，脚本会去改文件里的下一个 `image:`——即误改 jaeger 的镜像行、把它也变成 `server_local` 导致 jaeger 启动崩。安全阀：server 段显式写 `image: xiaozhi-esp32-server:server_local`，脚本 grep 命中即整段跳过 sed，jaeger 行安全。
- **override 不在 sync-python.sh 白名单**：改了 override 必须手动 rsync 到 ECS，`sync-python.sh` 只同步 6 个代码子路径，不含 compose 文件。
